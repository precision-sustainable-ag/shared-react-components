...
const RegionSelectorMap = ({
  ...
  useEffect(() => {
    ...
    if (selectedStateId) {
      mapInstance.setFeatureState({ source: 'states', id: selectedStateId }, { click: false });
    }
    ...
    mapInstance.setFeatureState({ source: 'states', id: selectedStateId }, { click: true });
  ...
  useEffect(() => {
    ...
    mapInstance.addLayer({
      ...
      layout: {},
      paint: {
        'fill-color': ['case', ['boolean', ['feature-state', 'click'], false], '#000', '#ccc'],
    ...
    mapInstance.on('mousemove', 'state-fills', (e) => {
      if (e.features.length > 0) {
        if (hoveredStateId !== null) {
          mapInstance.setFeatureState({ source: 'states', id: hoveredStateId }, { hover: false });
        }
        hoveredStateId = e.features[0].id;
        setHoveredStateName(e.features[0].properties.STATE_NAME);
        mapInstance.setFeatureState({ source: 'states', id: hoveredStateId }, { hover: true });
      }
    });
    ...
    mapInstance.on('click', 'state-fills', (e) => {
      ...
      if (boundaryData && boundaryData.features) {
        let selectedFeature = boundaryData.features.filter((el) => el.id === selectedStateId);
